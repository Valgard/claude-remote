...
- **Env seams for testability:** all external-tool access goes through `CR_TMUX` (default `tmux`), `CR_ABTOP` (default `abtop`), `CR_SSH_PORT` (default `22`), `CR_ANCHOR` (default `_cr_anchor`, the keychain-anchor holding session). `CR_NEW_DIR` (default `${HOME}/Projects`) is the starting directory the picker's `＋ neue Session` entry offers when you press Enter at the directory prompt. Never call `tmux`/`abtop` directly in `lib/` — go through the variable. Tests override these to point at an isolated `tmux -L <socket>` server and stub binaries (`tests/fixtures/abtop-stub`, `tests/fixtures/fake-claude`), so the suite never touches the user's real sessions.
