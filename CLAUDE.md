...
bats-core suite under `tests/`, one file per concern (`join`, `render`, `pane_map`, `name`, `fallback`, `pick`, `launch`, `install`, `path`, `resolve`, `locale`, `sshd`, `abtop_parse`, `anchor`, `exit_output`). Each sources `tests/helpers.bash` via `load helpers`:
