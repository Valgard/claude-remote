...
bats-core suite under `tests/`, one file per concern (`join`, `render`, `pane_map`, `name`, `fallback`, `pick`, `launch`, `install`, `path`, `locale`, `sshd`, `abtop_parse`, `anchor`). Each sources `tests/helpers.bash` via `load helpers`:
